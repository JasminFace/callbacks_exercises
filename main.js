...
const sumPurchases = transactions.filter(transaction => transaction.type == 'purchase').flatMap(transaction => transaction.items).reduce(((total, item) => total + item.price), 0);
...
*/
const netProfit = transactions.flatMap(transaction => transaction.items).reduce(((total, item) => total + item.price), 0);
...
*/
const sumOfSmallestPurchase = transactions.filter(transaction => transaction.type == 'purchase').map(transaction => transaction.items).sort()[0].reduce(((total, item) => total + item.price), 0);
